...
这会生成 `${CLAUDE_PLUGIN_DATA}/grados-config.json`，并引导你填写 API Key。Plugin 还会自动把 `local-rag` 指向 `${CLAUDE_PLUGIN_DATA}/papers`，所以默认流程下不需要再配置环境变量或 shell profile。
...
> **重要：** `BASE_DIR` 必须和 `grados-config.json` 里的 `extract.papersDirectory` 指向同一个绝对目录。如果 `extract.papersDirectory` 写的是相对路径，请先按 `PROJECT_ROOT`（通常是配置文件所在目录）把它解析成绝对路径。
...
现在内置的 Puppeteer 回退已经支持 **Windows、macOS 和 Linux**，但只会探测你配置的浏览器类型（`msedge`、`chrome` 或 `firefox`）在当前系统上的常见路径；如果找不到，也可以在 `grados-config.json` 中显式设置 `headlessBrowser.executablePath`。
...
如果你希望一次性接入最常见的完整研究工作流，可以把这三个 MCP 服务一起配置。下面这个例子假设配置文件在 `D:/Projects/Papers/grados-config.json`，并且 `extract.papersDirectory` 使用默认相对路径 `./papers`，因此 `mcp-local-rag` 的 `BASE_DIR` 对应到 `D:/Projects/Papers/papers`。
...
最终解析出的配置文件所在目录会成为 `PROJECT_ROOT`。`grados-config.json` 中所有相对路径（例如 `./papers`、`./downloads`）都会相对于这个目录解析，而不是相对于进程当前工作目录。
