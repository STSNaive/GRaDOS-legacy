...
[`mcp-local-rag`](https://github.com/shinpr/mcp-local-rag) 提供本地论文语义检索。GRaDOS 会自动把解析后的 Markdown 写入 `papers/`，而 `mcp-local-rag` 负责索引并检索该目录。它和 GRaDOS 一样是纯 Node.js，不需要 Python。
